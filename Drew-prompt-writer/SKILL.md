...
每个分镜头以一整段连贯文字输出，专注于拍摄技术要素，不包含服装细节描述。所有分镜头并入总提示词中作为镜头描述部分。

**分镜头核心要素**（按顺序描述）：
1. **机位** - 拍摄高度和角度（Eye level / High angle / Low angle）
2. **景别** - 镜头类型（Wide Shot / Medium Shot / Close-up）
3. **运镜方式** - 相机运动（Static / Tracking / Dolly / Pan）
4. **角色位置** - 在画面中的位置（Left third / Center / Right third）
5. **角色行为动作** - 具体动作描述（不含服装细节）
6. **构图** - 构图原则（Rule of thirds / Leading lines / Negative space）
7. **镜头时长** - 持续时间（X seconds duration）

**分镜头段落示例**:
```
(分镜头1) SHOT 01, Eye level / Wide angle, Wide Shot (WS) establishing environment, Static camera mounted on tripod, subject positioned on left third of frame, standing in relaxed stance preparing for action, rule of thirds composition with generous headroom and environmental context, natural daylight with soft shadows, 4-5 seconds duration, slow motion 60fps+

(分镜头2) SHOT 02, Eye level / Side angle, Medium Shot (MS) knee-up framing, slow tracking shot following subject's movement, subject center-frame moving from left to right, smooth motion with natural arm swing and confident posture, slight Dutch angle enhancing dynamic feeling, golden hour sunlight with rim light on subject, 5-6 seconds duration, slow motion with subtle camera movement

(分镜头3) SHOT 03, Eye level / Front angle, Close-up (CU) waist-up framing, static camera with slight breathing movement acceptable, subject centered in frame, subtle head tilt with focused expression and gentle smile, face following rule of thirds with shallow depth of field, soft frontal key light with catch lights in eyes, 3-4 seconds duration, cinematic film-like quality
```
...
(分镜头1) SHOT 01, Eye level / Wide angle, Wide Shot (WS) establishing environment, Static camera mounted on tripod, subject positioned on left third of frame near large window, standing in profile with serene expression and slight sway beginning to turn toward camera, rule of thirds composition with window providing context and depth, generous headroom and environmental context, golden hour sunlight through window with warm tones and soft shadows, 4-5 seconds duration, slow motion 60fps+, professional cinematography

(分镜头2) SHOT 02, Eye level / Front angle, Medium Shot (MS) waist-up framing, slow dolly in tracking with subject's turning motion, subject center-frame turning toward camera, smooth turning motion with hair cascading and eyes meeting camera lens and soft gentle smile forming, center framed with slight breathing room above head and tracking movement left to right, Rembrandt-style key light from window with soft fill on opposite side and glowing rim light, 5-6 seconds duration, slow motion with subtle camera movement, shallow depth of field, bokeh background

(分镜头3) SHOT 03, Eye level / Straight-on angle, Close-up (CU) face and upper chest framing, static camera with slight natural breathing movement acceptable, subject centered in frame, gentle smile fully developed with eyes opening slightly wider and subtle head tilt looking directly into lens, face following rule of thirds with eyes on upper horizontal line and slight negative space below chin, soft frontal key light with subtle catch lights in eyes and shallow depth of field emphasizing eyes, 3-4 seconds duration, film-like quality, professional color grading, romantic mood
...
本技能采用**模块化 + 灵活组合**的版式设计，确保跨设备一致性的同时保留创作灵活性。
...
| 段落标识 | 内容维度 | 出现条件 | 示例 |
|----------|----------|----------|------|
| `(主体)` | 人物/物体核心属性 | **必须** | age, ethnicity, appearance |
| `(服装)` | 穿着描述 | 有服装时 | wearing [outfit from image 1] |
| `(配饰)` | 配饰/道具 | 有配饰时 | carrying [bag from image 2] |
| `(动作)` | 行为/动态 | 有动作时 | walking, dancing, posing |
| `(场景)` | 环境/地点 | **必须** | outdoor, beach, studio |
| `(构图)` | 画面布局 | **必须** | wide shot, 16:9, rule of thirds |

**技术段落（按逻辑排序）**:

| 段落标识 | 内容维度 | 出现条件 | 示例 |
|----------|----------|----------|------|
| `(镜头)` | 焦段/镜头类型 | 有指定时 | 50mm lens, telephoto |
| `(技术)` | 拍摄参数 | 有指定时 | aperture, shutter speed |
| `(光照)` | 光线设计 | **推荐** | natural daylight, soft shadows |
| `(氛围)` | 情绪/风格 | **推荐** | cinematic, warm tones |
| `(保护)` | 一致性保护 | 有图片引用时 | strictly follow image design |
| `(质量)` | 画质标签 | **必须** | 4k, professional photography |

**视频专属段落**:

| 段落标识 | 内容维度 | 出现条件 | 示例 |
|----------|----------|----------|------|
| `(分镜头N)` | 第N个镜头描述 | 有分镜头指令时 | SHOT 01, camera angle... |
| `(速度)` | 帧率/慢动作 | 有指定时 | slow motion, 60fps+ |
| `(时长)` | 视频长度 | 有指定时 | 5 seconds duration |
...
- 分镜头段落专注于拍摄技术要素
- 不包含服装/配饰细节（已在主体段落描述）
- 按 `(分镜头1)` → `(分镜头2)` → `(分镜头3)` 顺序排列

### 版式示例

**图生图标准版式**:
```
(主体) → (服装) → (配饰) → (动作) → (场景) → (构图) → (镜头) → (技术) → (光照) → (氛围) → (保护) → (质量)
```

**图生视频标准版式**:
```
(主体) → (服装) → (配饰) → (动作) → (场景) → (构图) → (分镜头1) → (分镜头2) → ... → (技术) → (光照) → (氛围) → (保护) → (质量)
```

**简化版式（无服装/配饰）**:
```
(主体) → (动作) → (场景) → (构图) → (技术) → (光照) → (氛围) → (质量)
```
